...
{
	public class RemoteFunction_CouponTranCommit : RemoteFunction
	{
		public override Message Process()
		{
			...
			switch (Coupon.Commit_UsingTransaction(couponid, account, characterid, server))
			{
		...
		protected override string BuildName(Message _input)
		{
			...
			return "Coupon Using Transaction Commit (coupon:" + text + "@" + text2 + ", target:<" + num + ">@" + text3 + ")";
		}
	}
}
