...
{
	public class RemoteFunction_PetCardTranRollback : RemoteFunction
	{
		public override Message Process()
		{
			...
			if (PetCard.Rollback_UsingTransaction(account, cardid, petid))
			{
				newReply.WriteU8(1);
			}
			else
		...
		protected override string BuildName(Message _input)
		{
			...
			return "Pet Card Using Transaction Rollback (card:" + num + "@" + text + ", target:<" + num2 + ">)";
		}
	}
}
