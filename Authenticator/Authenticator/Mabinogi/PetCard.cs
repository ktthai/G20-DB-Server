...
{
    public class PetCard : CardInterface
    {
        ...
        protected static SimpleConnection Connection
        {
            get
            {
                ...
                {
                    return new SQLiteSimpleConnection(ServerConfiguration.PetCardConnectionString);
                }
                else
                {
                    return new MySqlSimpleConnection(ServerConfiguration.PetCardConnectionString);
                }
            }
        }
        ...
        {
            WorkSession.WriteStatus("PetCard.ToMessage() : enter");
            ...
            if (m_ValidServers != null)
            {
                ...
                string[] validServers = m_ValidServers;
                foreach (string CardServer in validServers)
                {
                    message.WriteString(CardServer);
                }
            }
            else
            {
            ...
            WorkSession.WriteStatus("PetCard.ToMessage() : complete, return msg");
            return message;
        }

        public static PetCard[] Load(string _account)
        {
            WorkSession.WriteStatus("PetCard.Load(\"" + _account + "\") : enter");
            try
            {
                WorkSession.WriteStatus("PetCard.Load(\"" + _account + "\") : open db connection");

                using (var conn = Connection)
                using (var cmd = conn.GetDefaultSelectCommand(Tables.Shop.PetCards))
                {
            ...
            {
                WorkSession.WriteStatus("PetCard.Load(\"" + _account + "\") : close db connection");
            }
            return new PetCard[0];
        }
        ...
        public static bool Begin_UsingTransaction(string _account, long _cardid, long _entityId, string _entityName, int _rebirthcount, string _server, string _comment)
        {
            ...
            using (var cmd = conn.GetDefaultUpdateCommand(Tables.Shop.PetCards, trans))
            {
        ...
        public static bool Commit_UsingTransaction(string _account, long _cardid, long _entityid)
        {
            ...
            using (var conn = Connection)
            using (var trans = conn.BeginTransaction())
            using (var cmd = conn.GetUpdateCommand($"UPDATE `{Tables.Shop.PetCards}` SET `{Columns.Cards.Status}` = @{Columns.Cards.Status} WHERE `{Columns.Cards.Id}` = @{Columns.Cards.Id} " +
        ...
        public static bool Rollback_UsingTransaction(string _account, long _cardid, long _petid)
        {
            ...
            using (var conn = Connection)
            using (var trans = conn.BeginTransaction())
            using (var readCmd = conn.GetDefaultSelectCommand(Tables.Shop.PetCards, trans))
            {
                try
                {
            ...
            using (var trans = conn.BeginTransaction())
            {
                if (ServerConfiguration.IsLocalTestMode)
                {
                    using (var cmd = conn.GetInsertCommand(string.Format("INSERT INTO `{0}`(`{1}`,`{2}`,`{3}`,`{4}`,`{5}`) VALUES(@{1},@{2},@{3},@{4},(SELECT COUNT(*) FROM `{0}`))",
                        /*0*/ Tables.Shop.PetCards, /*1*/ Columns.Cards.Id, /*2*/ Columns.Cards.Created, /*3*/ Columns.Cards.Type, /*4*/ Columns.Cards.ValidServer, /*5*/ Columns.Cards.CardId), trans))
                    {
                ...
                {
                    using (var cmd = conn.GetDefaultInsertCommand(Tables.Shop.PetCards, trans))
                    {
            ...
            try
            {
                ...
                using (var conn = Connection)
                {
                    if (ServerConfiguration.IsLocalTestMode)
                    {
                        ...
                        /*0*/ Tables.Shop.PetCards, /*1*/ Columns.Cards.Id, /*2*/ Columns.Cards.Type, /*3*/ Columns.Cards.CardId)))
                        {
                            cmd.AddParameter(Columns.Cards.Id, _account);
                            cmd.AddParameter(Columns.Cards.Type, _type);
                    ...
                    {
                        using (var cmd = conn.GetDefaultInsertCommand(Tables.Shop.PetCards))
                        {
                            cmd.Set(Columns.Cards.Id, _account);
                            cmd.Set(Columns.Cards.Type, _type);
